...
// Adds (or subtracts, via a negative value) whole calendar months to a UTC
// date the same way spreadsheet EDATE() does: if the resulting month is
// shorter than the original day of month, the day is clamped down to that
// month's last day (e.g. Jan 31 + 1 month = Feb 28, not Mar 3).
function addMonths(date: Date, monthsToAdd: number): Date {
  ...
  return new Date(Date.UTC(newYear, newMonthIndex, clampedDay));
}
...
export default function MonthsFromTodayCalculator() {
  ...
  useEffect(() => {
    ...
    if (sharedMonths && sharedDate) {
      ...
      const saved = history["months-from-today-calc"]?.data;
      if (saved) {
  ...
  useEffect(() => {
    saveCalculatorHistory("months-from-today-calc", {
      months,
      startDate,
      direction,
    });
  }, [months, startDate, direction]);
  ...
  // --- Core calculation ---
  const results = useMemo<MonthsFromResult | null>(() => {
    if (trigger === 0) return null;

    if (!startDate || months === "") {
      return {
        error: "Please enter a number of months and a date.",
    ...
    if (!base || isNaN(monthCount) || !Number.isInteger(monthCount)) {
      return {
        error: "Please enter a whole number of months.",
        formats: buildFormats(new Date()),
        totalDays: 0,
        wasClamped: false,
      };
    ...
    const delta = direction === "subtract" ? -Math.abs(monthCount) : Math.abs(monthCount);
    const resultDate = addMonths(base, delta);

    // Flag when the day had to be pulled back to fit a shorter month
    // (e.g. Jan 31 + 1 month landing on Feb 28), since that trips people up.
    const wasClamped = resultDate.getUTCDate() !== base.getUTCDate();

    const totalDays = Math.round(
      Math.abs(resultDate.getTime() - base.getTime()) / 86400000
    );

    return { formats: buildFormats(resultDate), totalDays, wasClamped };
  }, [trigger, months, startDate, direction]);
  ...
  }, [showResults, results, months, startDate, direction]);
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-card rounded-2xl border p-5 md:p-8 shadow-sm relative overflow-hidden">
              ...
              <button
                ...
                aria-label={
                  isSaved
                    ? "Remove Months From Today Calculator from saved"
                    : "Save Months From Today Calculator"
                }
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  <label
                    ...
                    className="text-[10px] font-black uppercase text-gray-300 tracking-widest"
                  >
                    {direction === "add" ? "Add months to" : "Subtract months from"}
                  </label>
                  <input
                    id="start-date"
                    type="date"
              ...
              <div className="bg-card border-2 border-blue-600/20 rounded-3xl p-6 md:p-12 shadow-sm animate-in fade-in zoom-in-95 duration-300">
                <div className="space-y-2 text-center">
                  ...
                  <div className="inline-block px-4 py-2 bg-blue-50 text-blue-700 rounded-full text-xs font-bold mt-4">
                    ≈ {results.totalDays.toLocaleString()} days {direction === "subtract" ? "earlier" : "later"}
                  </div>
                  {results.wasClamped && (
                    <p className="text-xs text-gray-300 pt-2 max-w-md mx-auto">
                      Note: the starting day of month doesn&apos;t exist that many months {direction === "add" ? "ahead" : "back"}, so it was pulled back to the last day of that month.
                    </p>
                  )}
                </div>
                  ...
                  {!shareUrl ? null : (
                    <div className="space-y-3">
                      ...
                      <p className="text-xs text-gray-300">
                        Anyone who opens this link sees the same months, direction, and date.
                      </p>
                    </div>
                  )}
                </div>
