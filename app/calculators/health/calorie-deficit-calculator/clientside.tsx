...
import { Target, Zap, Flame, Scale, RotateCcw, Apple, Beef, Wheat, CheckCircle2, ListFilter, Info, HelpCircle } from 'lucide-react';
      ...
      <section className="py-8 px-4 max-w-7xl mx-auto">
            ...
            <div className="bg-card rounded-xl border p-6 shadow-sm">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="number" value={tdee} onChange={(e) => setTdee(e.target.value)} className="w-full mt-1 px-3 py-3 bg-secondary rounded-md border font-bold text-lg" />
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-2">
                    {['lose', 'maintain', 'gain'].map((g) => (
                      <button key={g} onClick={() => setGoal(g as any)} className={`py-2 rounded-md text-xs font-bold capitalize border transition-all ${goal === g ? 'bg-blue-600 text-white border-blue-600' : 'bg-secondary'}`}>
                        {g}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-2 mt-2">
                    {[
                      ...
                      <button key={d.id} onClick={() => setDietType(d.id as any)} className={`flex items-center gap-3 p-3 rounded-md text-sm font-bold border transition-all ${dietType === d.id ? 'bg-blue-600/10 border-blue-600 text-blue-600' : 'bg-secondary border-transparent'}`}>
                ...
                <div className="pt-2">
                  <button onClick={handleCalculate} className="w-full py-3 bg-blue-600 text-white rounded-md font-bold text-sm hover:bg-blue-700 shadow-lg shadow-blue-500/20 transition-all flex items-center justify-center gap-2">
                    Calculate Macros <CheckCircle2 size={16} />
                  </button>
                  <button onClick={() => {setTdee('2500'); setGoal('maintain'); setDietType('balanced'); setResults(null);}} className="w-full mt-2 py-2 bg-secondary text-muted-foreground rounded-md font-bold text-xs hover:bg-secondary/80 transition-all flex items-center justify-center gap-2">
          ...
          <div className="lg:col-span-8 space-y-6">
            {results ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-blue-600 text-white rounded-xl p-8 shadow-lg flex flex-col justify-center">
                  <p className="text-xs font-bold uppercase opacity-80 tracking-widest">Target Daily Intake</p>
                  <h2 className="text-5xl font-black my-4">{results.totalCals} <span className="text-lg font-normal opacity-80">kcal</span></h2>
                  <p className="text-sm font-medium">For {goal}ing weight on a {dietType} diet</p>
                </div>
                  ...
                  {[
                    ...
                    <div key={m.label} className="bg-card border p-4 rounded-xl">
                      ...
                      <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
                        <div style={{ width: `${m.width * 100}%` }} className={`h-full ${m.bg}`} />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-secondary/20 border-2 border-dashed border-border rounded-xl p-12 text-center h-full flex flex-col items-center justify-center">
                ...
                <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest">Configure settings and click Calculate</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-card border rounded-xl p-8">
              <h3 className="font-bold text-xl mb-4 flex items-center gap-2"><Info size={20} className="text-blue-600"/> Why Macros Matter</h3>
            ...
            <div className="bg-card border rounded-xl p-8">
              <h3 className="font-bold text-xl mb-4 flex items-center gap-2"><HelpCircle size={20} /> Popular Diet Splits</h3>
              <p className="text-sm text-muted-foreground">Adjust your intake based on your specific needs—like high protein for muscle growth or Keto for metabolic adaptation.</p>
            </div>
        </div>
