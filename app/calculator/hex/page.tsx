...
import { Code, RotateCcw, Info, ChevronRight, Calculator, Hash, Zap, CheckCircle2, Cpu, Terminal } from 'lucide-react'
...
export default function HexCalculator() {
  ...
  // --- Input States ---
  const [valueA, setValueA] = useState<string>('1A')
  const [valueB, setValueB] = useState<string>('F2')
  ...
  const results = useMemo((): HexResult | { error: string } | null => {
    ...
    try {
      ...
      if (isNaN(numA) || isNaN(numB)) return { error: "Invalid Hexadecimal input." }
      ...
      if (resNum < 0) return { error: "Negative results are not supported in this mode." }

      return {
        hex: resNum.toString(16).toUpperCase(),
        decimal: resNum.toString(10),
        binary: resNum.toString(2).padStart(8, '0'),
        operation: `0x${valueA.toUpperCase()} ${operator} 0x${valueB.toUpperCase()}`
      }
    } catch (e) {
      return { error: "An unexpected calculation error occurred." }
    }
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 md:py-16">
        <div className="mb-12 text-center md:text-left">
          <h1 className="text-4xl md:text-6xl font-black mb-4 tracking-tighter uppercase">
            Hex <span className="text-primary">Calculator</span>
          </h1>
          <p className="text-lg text-muted-foreground max-w-2xl leading-relaxed">
            Perform arithmetic operations on hexadecimal values. Essential for web design, debugging, and system architecture.
          </p>
        </div>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-500">
                <div className="bg-primary rounded-3xl p-8 text-primary-foreground shadow-2xl relative overflow-hidden">
                  <div className="relative z-10">
                    <span className="text-[10px] font-bold uppercase tracking-widest opacity-70">Hex Result</span>
                    ...
                    <div className="mt-6 p-4 bg-white/10 rounded-2xl backdrop-blur-sm border border-white/10">
                        <p className="text-[10px] uppercase font-bold opacity-60 mb-1">Decimal Equivalent</p>
                ...
                <div className="bg-card border border-border rounded-3xl p-6 shadow-sm">
                   <h4 className="text-[10px] font-black uppercase text-muted-foreground mb-4 tracking-widest">Data Representation</h4>
                   <div className="space-y-3">
                      <StatRow label="Binary Bits" value={results.binary} />
                      <StatRow label="Last Action" value={results.operation} />
                   </div>
                </div>
              </div>
            ) : (
              <div className="bg-muted/30 border-2 border-dashed border-border rounded-3xl p-12 text-center flex flex-col items-center justify-center min-h-[400px]">
                {results && 'error' in results ? (
                    ...
                    <>
                        ...
                        <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Awaiting Hex Data</p>
                    </>
                )}
              </div>
            )}
            ...
            <section className="bg-card rounded-3xl border border-border p-6 md:p-10 shadow-xl relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-10">
                  ...
                  <h2 className="text-xl font-bold tracking-tight">Base-16 Logic</h2>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                  <div className="md:col-span-5">
                    <InputField label="Hex A" value={valueA} onChange={(v) => {setValueA(v); setShowResults(false)}} />
                  </div>
                  ...
                  <div className="md:col-span-5">
                    <InputField label="Hex B" value={valueB} onChange={(v) => {setValueB(v); setShowResults(false)}} highlight />
                  </div>
                </div>
                ...
                <div className="mt-10 flex flex-col md:flex-row gap-4">
                  ...
                  <button 
                    onClick={() => {setShowResults(false); setValueA(''); setValueB(''); setOperator('+');}}
        ...
        <section className="mt-16 bg-card rounded-3xl border border-border p-8 md:p-12 shadow-sm">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div className="space-y-6">
                    <h3 className="text-2xl font-black uppercase tracking-tight">Understanding Hexadecimal</h3>
                    <p className="text-muted-foreground text-sm leading-relaxed">
                        Hexadecimal is a 16-base positional numeral system. It uses sixteen distinct symbols: the numbers <strong>0–9</strong> and the letters <strong>A–F</strong> to represent values from ten to fifteen.
                    </p>
                    
                    <div className="p-4 bg-primary/5 border border-primary/20 rounded-2xl">
                        <code className="text-primary font-bold text-sm">Color Example: #FFFFFF = (255, 255, 255) in RGB</code>
                    </div>
                </div>
                <div className="space-y-6">
                    <h3 className="text-2xl font-black uppercase tracking-tight">Why Programmers Love Hex</h3>
                    <div className="space-y-4">
                        <TipItem text="Conciseness: One hex digit represents exactly four binary bits (a nibble)." />
                        <TipItem text="Memory Mapping: Hex is the standard for displaying memory addresses in debuggers." />
                        <TipItem text="Web Design: CSS uses hex codes to define over 16 million colors efficiently." />
                    </div>
                </div>
            </div>
        </section>
        ...
        <RelatedCalculators calculators={[
          { name: 'Binary Calculator', description: 'Base-2 arithmetic', href: '/calculator/binary', icon: Cpu },
          { name: 'Color Converter', description: 'Hex to RGB and HSL', href: '/calculator/color', icon: Zap }
        ]} />
      </div>
      <Footer />
    </main>
...
function InputField({ label, value, onChange, highlight }: { label: string, value: string, onChange: (v: string) => void, highlight?: boolean }) {
  return (
    <div className="relative group">
      ...
      <input 
        type="text" value={value} onChange={(e) => onChange(e.target.value.replace(/[^0-9a-fA-F]/g, '').toUpperCase())}
