...
//       case 'asin': input('asin('); break
//       case 'acos': input('acos('); break
//       case 'atan': input('atan('); break
//       case 'sqrt': input('√('); break
//       case 'log': input('log('); break
//       case 'ln': input('ln('); break
//       case 'pi': input('π'); break
//       case 'x²': input('^2'); break
//       case '1/x': input('1/('); break
//       case 'exp': input('exp('); break
//     }
//   }
...
//         exp = exp.replace(/Math\.asin\(([^)]+)\)/g, '(Math.asin($1)*180/Math.PI)')
//         exp = exp.replace(/Math\.acos\(([^)]+)\)/g, '(Math.acos($1)*180/Math.PI)')
//         exp = exp.replace(/Math\.atan\(([^)]+)\)/g, '(Math.atan($1)*180/Math.PI)')
//       }
...
//           <h1 className="text-4xl font-bold mt-4 mb-6">Scientific Calculator</h1>

//           <div className="flex justify-center">
//             <div className="bg-card border border-border rounded-3xl p-6 w-[380px] shadow-2xl">

//               {/* Dual display */}
//               <div className="bg-background p-2 rounded mb-2 text-sm text-muted-foreground overflow-x-auto">{expression || '0'}</div>
//               <div className="bg-background p-4 rounded mb-4 text-3xl font-bold text-foreground overflow-x-auto">{display}</div>
...
//                 <button className={`font-bold text-white py-2 rounded ${angleMode==='DEG'?'bg-green-500':'bg-red-500'}`} onClick={()=>setAngleMode(angleMode==='DEG'?'RAD':'DEG')}>{angleMode}</button>
//               </div>

//               {/* Scientific buttons */}
//               <div className="grid grid-cols-5 gap-2 mb-3">
//                 {['sin','cos','tan','asin','acos','atan','sqrt','log','ln','pi','x²','1/x','exp','(',' )'].map(btn => (
//                   <button key={btn} className="btn-func" onClick={()=>sci(btn)}>{btn==='asin'?'sin⁻¹':btn==='acos'?'cos⁻¹':btn==='atan'?'tan⁻¹':btn}</button>
//                 ))}
//               </div>
...
//                   <button
//                     key={btn}
//                     className={btn==='='?'btn-equal':/[/*\-+]/.test(btn)?'btn-op':'btn-num'}
...
//       <style jsx>{`
//         .btn-calc { background:#a0a0a0; color:black; font-weight:bold; padding:0.5rem; border-radius:0.5rem; transition:all 0.1s; }
//         .btn-calc:hover { background:#b5b5b5; }
//         .btn-func { background:#4a90e2; color:white; font-weight:bold; padding:0.5rem; border-radius:0.5rem; transition:all 0.1s; }
//         .btn-func:hover { background:#357ab8; }
//         .btn-num { background:#333; color:white; font-weight:bold; padding:0.75rem; border-radius:0.5rem; transition:all 0.1s; }
//         .btn-num:hover { background:#444; }
//         .btn-op { background:#555; color:white; font-weight:bold; padding:0.75rem; border-radius:0.5rem; transition:all 0.1s; }
//         .btn-op:hover { background:#666; }
//         .btn-equal { background:#facc15; color:black; font-weight:bold; padding:0.75rem; border-radius:0.5rem; transition:all 0.1s; }
//         .btn-equal:hover { background:#eab308; }
//       `}</style>
//     </main>
//   )
// }
